...
/**
 * Identical to be.kuleuven.econ.heuristicsminer. HeuristicsMiner, but without
 * any need for a PluginContext.
 */
public class HeuristicsMinerNoUI {
	...
	public HeuristicsMinerNoUI(XLog log) {
	...
	public HeuristicsMinerNoUI(XLog log, XLogInfo logInfo) {
	...
	public HeuristicsMinerNoUI(XLog log, XLogInfo logInfo, HeuristicsMinerSettings settings) {
	...
	public HeuristicsMinerNoUI(XLog log, HeuristicsMinerSettings settings) {
			...
			for (int i = 0; i < metrics.getEventsNumber(); i++) {
				...
				if (i != metrics.getBestStart()) {
					...
					if ((j > -1) && (metrics.getBestInputMeasure(j) > metrics.getBestInputMeasure(i))) {
						...
						metrics.setNoiseCounters(bestInputEvent, i,
								metrics.getDirectSuccessionCount(i, bestInputEvent));
					}
				}
...
//		fitness1.calculate(population);
	...
	private double calculateL2LDependencyMeasure(int i, int j, HeuristicsMetrics metrics) {
		...
		if (!((metrics.getL1Lrelation(i) && (successionCountIJ >= threshold))
				|| (metrics.getL1Lrelation(j) && (successionCountJI >= threshold)))) {
	...
	private double andInMeasureF(int ownerE, int oldE, int newE, HeuristicsMetrics metrics) {
		if (ownerE == newE) {
			...
			// relevantInObservations;
					(metrics.getDirectSuccessionCount(newE, ownerE) + metrics.getDirectSuccessionCount(oldE, ownerE)
							+ 1);
		}
	}
	...
	private double andOutMeasureF(int ownerE, int oldE, int newE, HeuristicsMetrics metrics) {
		if (ownerE == newE) {
			...
			// relevantOutObservations;
					(metrics.getDirectSuccessionCount(ownerE, newE) + metrics.getDirectSuccessionCount(ownerE, oldE)
							+ 1);
		}
	}
	...
	public boolean escapeToEndPossibleF(int x, int y, BitSet alreadyVisit, HeuristicsNet net) {
		...
		// search for minNum of elements of min subset with X=B as element: [A B] ,
		// minNum = 2
